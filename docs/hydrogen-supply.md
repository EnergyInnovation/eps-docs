...
Revenues going to non-energy industries are allocated to some of the 42 ISIC codes tracked in the model. (For more detail on ISIC codes, see the documentation page on the [input-output model](io-model).) The shares for labor and for government (i.e. taxes) are separated out later, on the [Cross-Sector Totals sheet](cross-sector-totals). Changes in capital expenditures are allocated based on the input data provided in `hydgn/SoHPCCbRIC` Share of Hydrogen Production Capital Costs by Recipient ISIC Code, while changes in non-fuel operational expenditures are allocated to ISIC 31T33 (Other manufacturing; repair and installation of machinery and equipment).
