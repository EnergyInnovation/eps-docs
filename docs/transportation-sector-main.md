...
<!-- TODO: verify framing of the BEV-share → effective electricity price feedback. Because the public/private charging mix for passenger LDVs is determined by the previous year's BEV stock share, growth in BEVs lowers the effective electricity price faced by the next round of buyers, which in turn (via lower fuel cost per unit cargo distance) feeds back into the vehicle choice function. Confirm this characterization is consistent with the intended model behavior. -->
